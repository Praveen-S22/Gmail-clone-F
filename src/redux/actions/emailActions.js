...
export const getEmailsAction = () => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch({ type: FETCH_EMAILS_SUCCESS, payload:  [ response.data.mailbox, response.data.email] });
  } catch (error) {
    dispatch({ type: FETCH_EMAILS_ERROR, error });
  }
};

export const sendEmailAction = (form, id, replyId) => async (dispatch, getState) => {
  dispatch({ type: SEND_EMAIL_REQUEST });
  try {
    const response = await sendEmail(getState().userReducer.token, form, id, replyId);
    dispatch({
      type: SEND_EMAIL_SUCCESS,
      payload: { outbox: response.data.sent },
    });
  } catch (error) {

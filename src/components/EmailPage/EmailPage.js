...
export default function EmailPage() {
  ...
  const [outboxReply, setOutboxReply] = useState([]);
  ...
  useEffect(() => {
    ...
    let inboxArr = [...mailbox.inbox],
      ...
      starredArr = [...mailbox.favorite],
      inboxReplyArr = mailbox.inboxReply,
      outboxReplyArr = mailbox.outboxReply,
      readArr = mailbox.read;
    ...
    setEmailArr(email);
    setInboxReply(inboxReplyArr);
    setOutboxReply(outboxReplyArr);
    setRead(readArr);
  }, [mailbox, email]);
  ...
  const [composeReplyDraft, setComposeReplyDraft] = useState(undefined);
  const [id, setId] = useState(undefined);

  const toggleShowSidebar = () => setShowSidebar(!showSidebar);
  const toggleIsCompose = (id, isDraft, Mails) => {
    ...
    if (id && isDraft == true) {
      ...
      setId(id);
      Mails.forEach((mail) => mail._id === isDraft && setComposeReplyDraft(mail));
    }
    else {
      setComposeDraft(undefined);
      setComposeReplyDraft(undefined);
    }
  };
      ...
      <main className={styles.main}>
        ...
        <div className={styles.mailContainer}>
          <Route path='/email/:category/view/:id'>
            <EmailView inbox={inbox} sent={sent} drafts={drafts} starred={starred} trash={trash} read={read} email={emailArr} inboxReply={inboxReply} outboxReply={outboxReply} toggleIsCompose={toggleIsCompose} />
          </Route>
        </div>

        {isCompose && <ComposeMail toggleIsCompose={toggleIsCompose} composeDraft={composeDraft} id={id} composeReplyDraft={composeReplyDraft} />}
      </main>
    </Fragment>
  );
}
